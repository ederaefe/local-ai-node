...
### 8. Headless Automation, Multi-Interface Resolution & Binary GGUF Introspection
- **Dual-Mode Dispatch Engine (`argparse`)**: Supports full unattended execution (`--model`, `--port`, `--threads`, `--daemon`, `--chat`, `--bench`, `--status`, `--test`, `--kill-zombies`). When invoked without arguments, boots into the interactive TUI; when supplied with flags, functions as a headless background daemon suitable for startup shortcuts, scripts, and service managers.
- **Multi-Adapter Interface Resolution (`get_all_lan_ips`)**: Enumerates all host network interfaces and automatically prioritizes physical subnets (`192.168.x.x`, `10.x.x.x`) over internal virtual bridge adapters (`172.x.x.x` from WSL/Hyper-V). Provides an interactive adapter selector (`Option 7`) to designate the broadcast IP.
- **Pure-Python Binary GGUF Header Inspector (`read_gguf_metadata`)**: Parses binary GGUF headers (little-endian byte streams: magic `GGUF`, version, tensor counts, and metadata key-value pairs) directly using Python's `struct` library without external dependencies. Introspects model architecture, parameter count, and context length.
- **In-Terminal Multi-Turn Chat Client (`interactive_server_chat`)**: Exposes an interactive conversation shell (`Option 4 -> [c]`) communicating directly with the background `llama-server` via `/v1/chat/completions`. Maintains conversation history and reports real-time latency, token counts, and generation throughput (`tokens/sec`).
